...
//!
//! These tests read the workflows and the manifest textually, as
//! `makefile_contract.rs` reads the `Makefile`, and hold the split:
//!
...
//! manifest directory, as in `makefile_contract.rs`.
...
fn workflows() -> std::io::Result<Vec<(String, String)>> {
    ...
    for entry in dir.entries()? {
        ...
        let is_workflow = camino::Utf8Path::new(&name)
            .extension()
